...
public class LoginPage {
    ...
    public void initializeComponents() {
        ...
        //region Icon
        frame.setIconImage(new ImageIcon(ClientApp.logoIMG).getImage());
        //endregion
        ...
        //region Side Panel
        side = new JPanel();
        side.add(new JLabel(new ImageIcon(ClientApp.logoIMG)), "grow");
        //endregion
        ...
        login.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ClientApp.logEmp = (Employee) ClientApp.client.findMatch("Employee", new SQLCondBuilder("idNum", SQLCond.EQ, idTXT.getText(), SQLType.TEXT), new SQLCondBuilder("email", SQLCond.EQ, emailTxt.getText(), SQLType.TEXT));
                if (ClientApp.logEmp == null) {
                    ...
                    if (ClientApp.logEmp.getType().equals("Admin") || ClientApp.logEmp.getType().equals("Cashier") || ClientApp.logEmp.getType().equals("Manager")) {
                        JOptionPane.showMessageDialog(frame, "Login Successful!", "Login", JOptionPane.INFORMATION_MESSAGE);
                        ClientApp.client.setId(ClientApp.logEmp.getIdNum());
                        ClientApp.client.sendID();
                        frame.setVisible(false);
                        new MainApp(ClientApp.client);
                    } else {
                        JOptionPane.showMessageDialog(frame, "Your Username or password is incorrect! (" + (MAX_ATTEMPTS - totAttempts++) + " Attempts Remaining)", "Login", JOptionPane.INFORMATION_MESSAGE);
                    }
                }
