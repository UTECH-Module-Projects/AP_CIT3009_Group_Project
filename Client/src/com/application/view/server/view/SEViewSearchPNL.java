...
public class SEViewSearchPNL implements ActionListener, ChangeListener {
    ...
    private void initializeComponents() {
        ...
        typeCMB = new JComboBox<>(ClientApp.types);
        levelCMB = new JComboBox<>(ClientApp.levels);
        msgTXT = new JTextField();
        timeTXT = new JTextField();
        clientCMB = new JComboBox<>();

        print = new JButton("Print", new ImageIcon(ClientApp.printIMG));
        printAll = new JButton("Print All", new ImageIcon(ClientApp.printIMG));
    ...
    public void refresh() {
        clientCMB.setModel(new JComboBox<>(ClientApp.employees.map(Employee::getIdNum).toArray(String[]::new)).getModel());
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource().equals(typeCMB) || e.getSource().equals(levelCMB) || e.getSource().equals(clientCMB)) {
            ...
            for (int row : rows) {
                Log log = ClientApp.logs.get(seViewPNL.getLogTBL().convertRowIndexToModel(row));
                logs.add(log);
            }
