...
public class Server {
    ...
    public static GenericJPADAO<Log, String> logExeq;
    ...
    /**
     * The Path for the boot files
     */
    private static final String bootPath = "Server/src/com/database/boot/boot.txt";
    private static final String adminPath = "Server/src/com/database/boot/adminBoot.txt";
    private static final String depPath = "Server/src/com/database/boot/departmentBoot.txt";
    ...
    private void createTableExecutors() {
        ...
        invItemExeq = new GenericJPADAO<>(InvoiceItem.class, this.em);
        logExeq = new GenericJPADAO<>(Log.class, this.em);
    ...
    private void checkDatabaseBoot() throws RuntimeException {
        ...
        try (Scanner scanner = new Scanner(bootFile)) {
            ...
            if (!scanner.nextBoolean()) {
                ...
                try (Scanner depScanner = new Scanner(new File(depPath))) {
                    //Add All Department data to the database
                    while (depScanner.hasNextLine()) {
                        depExeq.create(new Department(depScanner.nextLine().split("\t")));
                    }
                ...
                try (Scanner adminScanner = new Scanner(new File(adminPath))) {
                    //Add Super Admin Record to the database
                    String[] str = adminScanner.nextLine().split("\t");
                    System.out.println(Arrays.toString(str));
                    empExeq.create(new Employee(str));
                    log.trace("Uploaded super admin data to database.");
                } catch (IOException e) {
                    log.fatal("I/O Exception! {" + e.getMessage() + "}");
                    throw new RuntimeException(e);
                }
