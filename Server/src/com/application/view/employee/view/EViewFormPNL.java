            ...
            if (showConfirmDialog(btnType + " Employee?") == JOptionPane.YES_OPTION) {
                if (isSave) {
                    employee = new Employee(
                            ...
                            ServerApp.departments.get(depCMB.getSelectedIndex())
                    );
                    try {
                        if (!employee.isValid()) {
                ...
                boolean result = (Boolean) client.cud(cudType, "Employee", (isSave ? employee : ServerApp.employees.get(empIndex).getIdNum()));
                String idNum = (isSave ? employee.getIdNum() : ServerApp.employees.get(empIndex).getIdNum());
                ...
                if (result) {
                    ...
                    if (isSave) {
                        if (isNew) {
                            ...
                            ServerApp.employees.add(employee);
                            eViewPNL.getEmpTBL().getTModel().addRow(employee.toArray());
                        } else {
                            ServerApp.employees.set(empIndex, employee);
                            eViewPNL.getEmpTBL().updateRow(empIndex, employee.toArray());
                        }
                    } else {
                        ServerApp.employees.remove(empIndex);
